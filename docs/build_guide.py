...
L["en"] = dict(
  ...
  a_intro="Two ways. The add-on is the simplest if you have a HomeWizard; the YAML works with <em>any</em> meter Home Assistant already shows you.",
  a1_t="With our add-on (no YAML)",
  a1=[
    ("Add the repository", f'In Home Assistant: <span class="k">Settings → Add-ons → Add-on Store → ⋮ (top right) → Repositories</span>. Paste this and press Add.', "code", REPO, "Repository address"),
    ("Install the add-on", 'It appears at the bottom of the store as <strong>GreenUtilityLog Meter Bridge</strong>. Open it and press <strong>Install</strong>.', None, None, None),
    ("Paste your token and start", 'On the <strong>Configuration</strong> tab paste your token into <span class="k">token</span>, press <strong>Save</strong>, then <strong>Start</strong> on the Info tab.', "see", "Check the Log tab", 'Within a minute you should see <span class="k">pushed 8421.3 kWh ✓</span>.'),
  ],
...
L["nl"] = dict(
  ...
  a_intro="Twee manieren. De add-on is het simpelst als je een HomeWizard hebt; de YAML werkt met <em>elke</em> meter die Home Assistant al laat zien.",
  a1_t="Met onze add-on (geen YAML)",
  a1=[
    ("Voeg de bron toe", f'In Home Assistant: <span class="k">Instellingen → Add-ons → Add-on Store → ⋮ (rechtsboven) → Repositories</span>. Plak dit en klik Toevoegen.', "code", REPO, "Adres van de bron"),
    ("Installeer de add-on", 'Die staat nu onderaan in de store als <strong>GreenUtilityLog Meter Bridge</strong>. Open ’m en klik <strong>Installeren</strong>.', None, None, None),
    ("Plak je token en start", 'Plak je token op het tabblad <strong>Configuratie</strong> bij <span class="k">token</span>, klik <strong>Opslaan</strong> en daarna <strong>Starten</strong> op het Info-tabblad.', "see", "Kijk op het Log-tabblad", 'Binnen een minuut hoort daar <span class="k">pushed 8421.3 kWh ✓</span> te staan.'),
  ],
...
L["de"] = dict(
  ...
  a_intro="Zwei Wege. Das Add-on ist am einfachsten, wenn du einen HomeWizard hast; das YAML funktioniert mit <em>jedem</em> Zähler, den Home Assistant bereits anzeigt.",
  a1_t="Mit unserem Add-on (kein YAML)",
  a1=[
    ("Repository hinzufügen", f'In Home Assistant: <span class="k">Einstellungen → Add-ons → Add-on Store → ⋮ (oben rechts) → Repositories</span>. Das hier einfügen und Hinzufügen drücken.', "code", REPO, "Repository-Adresse"),
    ("Add-on installieren", 'Es erscheint unten im Store als <strong>GreenUtilityLog Meter Bridge</strong>. Öffnen und <strong>Installieren</strong> drücken.', None, None, None),
    ("Token einfügen und starten", 'Im Tab <strong>Konfiguration</strong> den Token bei <span class="k">token</span> einfügen, <strong>Speichern</strong>, dann im Info-Tab <strong>Starten</strong>.', "see", "Schau ins Protokoll", 'Binnen einer Minute sollte dort <span class="k">pushed 8421.3 kWh ✓</span> stehen.'),
  ],
...
L["fr"] = dict(
  ...
  a_intro="Deux façons. L'add-on est le plus simple si vous avez un HomeWizard ; le YAML fonctionne avec <em>n'importe quel</em> compteur que Home Assistant affiche déjà.",
  a1_t="Avec notre add-on (sans YAML)",
  a1=[
    ("Ajoutez le dépôt", f'Dans Home Assistant : <span class="k">Paramètres → Modules → Boutique → ⋮ (en haut à droite) → Dépôts</span>. Collez ceci et appuyez sur Ajouter.', "code", REPO, "Adresse du dépôt"),
    ("Installez l'add-on", 'Il apparaît en bas de la boutique sous <strong>GreenUtilityLog Meter Bridge</strong>. Ouvrez-le et appuyez sur <strong>Installer</strong>.', None, None, None),
    ("Collez votre jeton et démarrez", 'Dans l\'onglet <strong>Configuration</strong>, collez le jeton dans <span class="k">token</span>, <strong>Enregistrer</strong>, puis <strong>Démarrer</strong> dans l\'onglet Info.', "see", "Regardez l'onglet Journal", 'En une minute vous devriez voir <span class="k">pushed 8421.3 kWh ✓</span>.'),
  ],
...
L["es"] = dict(
  ...
  a_intro="Dos formas. El add-on es lo más simple si tienes un HomeWizard; el YAML funciona con <em>cualquier</em> contador que Home Assistant ya muestre.",
  a1_t="Con nuestro add-on (sin YAML)",
  a1=[
    ("Añade el repositorio", f'En Home Assistant: <span class="k">Ajustes → Complementos → Tienda → ⋮ (arriba a la derecha) → Repositorios</span>. Pega esto y pulsa Añadir.', "code", REPO, "Dirección del repositorio"),
    ("Instala el add-on", 'Aparece al final de la tienda como <strong>GreenUtilityLog Meter Bridge</strong>. Ábrelo y pulsa <strong>Instalar</strong>.', None, None, None),
    ("Pega tu token e inicia", 'En la pestaña <strong>Configuración</strong> pega el token en <span class="k">token</span>, pulsa <strong>Guardar</strong> y luego <strong>Iniciar</strong> en la pestaña Info.', "see", "Mira la pestaña Registro", 'En un minuto deberías ver <span class="k">pushed 8421.3 kWh ✓</span>.'),
  ],
