...
.toast{position:fixed;top:72px;left:50%;transform:translateX(-50%);background:${T.text};border-radius:3px;padding:8px 14px;font-size:11px;font-weight:700;letter-spacing:.3px;color:${T.bg};z-index:400;white-space:nowrap;animation:toastin .18s ease;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;}
...
function BaselineOnboarding({ onDone, utils, existingBaselines, existingMeters, editMode, T }) {
  ...
  // Which inputs the user has visited and left. Validation styling keys off this:
  // an empty field nobody has touched yet is simply empty, not wrong.
  ...
  return (
    <div className="onboard">
          ...
          <div key={u.id} style={{display:"flex",alignItems:"flex-start",gap:12,background:T.card,borderRadius:4,padding:"10px 14px",border:`1px solid ${T.border}`}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <input
                ...
                onBlur={() => markTouched(`m:${u.id}`)}
                style={locked ? lockedInput : editInput((meters[u.id]||"").trim() || !needsMeter, `m:${u.id}`)}
              />
              {locked ? (
              ...
              <input
                ...
                onBlur={() => markTouched(`b:${u.id}`)}
                style={editInput(hasBaseline(u), `b:${u.id}`)}
              />
              )}
            </div>
          </div>
      ...
      {!allMetersFilled && <div style={{fontSize:11,color:Object.keys(touched).length ? "#e0553d" : T.textSoft,marginTop:6,textAlign:"center"}}>Enter a meter number <b>and</b> its current reading for every meter to continue.</div>}
