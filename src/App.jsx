...
// ═══════════════════════════════════════════════════════════════════════════════
// 🟢 GREEN UTILITY LOG - COMPLETE PRODUCTION-READY APP V3
// VeChain + VeBetterDAO Integration · May 2026
// ═══════════════════════════════════════════════════════════════════════════════
...
const PLAUSIBLE_RANGES = {
  electric: { min:0.1,  max:80  },
  gas:      { min:0.01, max:20  },
  water:    { min:10,   max:1000},
  solar:    { min:0.1,  max:60  },
};
...
function checkPlausibility(utilId, usageVal) {
  ...
  if (usageVal > range.max) return { ok:false, reason:`Abnormally high usage detected (${usageVal}). Flagged for review.` };
  return { ok:true };
}
...
function checkAnomaly(utilId, usageVal, subs) {
  ...
  if (usageVal > avg * 3.5) return { ok:false, anomaly:true, reason:`Usage is ${(usageVal/avg).toFixed(1)}× your personal average. Flagged.`, avg:parseFloat(avg.toFixed(2)) };
...
async function runOCR(file, claimedReading) {
  try {
    ...
    if (!match) {
      ...
      return { matched: false, ocrNums: nums, ocrConfidence: confidence, reason: `OCR reads ${nums.slice(0,3).join(", ")} — entered ${claimed}. Values don't match.` };
    }
...
const UTIL_ICONS = {
  ...
  gas:      <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" width="16" height="16"><path d="M10 17c-3.3 0-6-2.7-6-6 0-4 3-7 6-9 3 2 6 5 6 9 0 3.3-2.7 6-6 6z"/><path d="M10 13a2 2 0 000-4c-1.1 0-2 .9-2 2" strokeOpacity="0.6"/></svg>,
  water:    <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16"><path d="M10 2C7 6 4 9.5 4 13a6 6 0 0012 0c0-3.5-3-7-6-11z" opacity="0.9"/></svg>,
...
const ONBOARD_SLIDES = [
  ...
  { icon:<svg viewBox="0 0 32 32" fill="currentColor" width="40" height="40"><path d="M17 4L8 18h8l-1 10 9-14h-8l1-10z" opacity="0.9"/></svg>, title:"Electric Meter", sub:"LCD display showing kWh. Usually 3000–4000 range. Earn 0.61 B3TR/kWh", color:"#8a4200" },
  { icon:<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" width="40" height="40"><path d="M16 27c-5 0-9-4-9-9 0-6 4.5-10.5 9-13.5 4.5 3 9 7.5 9 13.5 0 5-4 9-9 9z" fill="currentColor" opacity="0.12"/><path d="M16 27c-5 0-9-4-9-9 0-6 4.5-10.5 9-13.5 4.5 3 9 7.5 9 13.5 0 5-4 9-9 9z"/><path d="M16 20a3 3 0 000-6c-1.7 0-3 1.3-3 3" opacity="0.5"/></svg>, title:"Gas Meter", sub:"Rotating dials or LCD in m³. Earn 0.84 B3TR/m³", color:"#7a1c1c" },
  { icon:<svg viewBox="0 0 32 32" fill="currentColor" width="40" height="40"><path d="M16 3C11 9 6 14 6 20a10 10 0 0020 0c0-6-5-11-10-17z" opacity="0.9"/></svg>, title:"Water Meter", sub:"Shows litres or m³. Often on outside wall. Earn 0.12 B3TR/L", color:"#10386a" },
  ...
  { icon:<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" width="40" height="40"><path d="M16 6l3 7h7l-5.5 4 2 7L16 20l-6.5 4 2-7L6 13h7z" fill="currentColor" opacity="0.15"/><path d="M16 6l3 7h7l-5.5 4 2 7L16 20l-6.5 4 2-7L6 13h7z"/></svg>, title:"Earn & Compete", sub:"Daily submissions build your streak. Climb the leaderboard and earn real B3TR.", color:"#3a1a6e" },
];
...
.lb-hero{margin:14px;border:1px solid ${T.border};border-left:3px solid ${T.green3};border-radius:4px;background:${T.card};padding:20px;}
...
.intro-hero{display:flex;flex-direction:column;align-items:center;gap:0;animation:slideup-fade .6s ease;}
.intro-icon{font-size:88px;margin-bottom:24px;animation:bounce 1.2s ease-in-out infinite;}
...
@keyframes slideup-fade{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}
...
// ═══════════════════════════════════════════════════════════════════════════════
// COMPONENT DEFINITIONS (In correct order)
// ═══════════════════════════════════════════════════════════════════════════════

function IntroScreen({ onStart }) {
  const slides = [
    { icon: '🌍', title: 'Track Your Home Utilities', sub: 'Photograph your electric, gas, water & solar meters daily.' },
    { icon: '💚', title: 'Reduce Your Footprint', sub: 'See your consumption trends and make smarter energy choices.' },
    { icon: '💰', title: 'Earn B3TR Rewards', sub: 'Get paid in real cryptocurrency for every meter you log on VeChain.' },
    { icon: '🏆', title: 'Join the Community', sub: 'Compete on global leaderboards and unlock exclusive benefits.' },
  ];
  ...
  return (
    <div className="intro-screen">
      ...
      <button className="intro-btn" onClick={() => isLast ? onStart() : setSlide(s => s + 1)}>
        {isLast ? 'Get Started' : 'Continue →'}
      </button>
      
      {!isLast && <button className="intro-skip" onClick={onStart}>Skip intro</button>}
    </div>
  );
}
      ...
      <div className="vz-result">
        ...
        {result.anomCheck?.anomaly && (
          <div style={{marginTop:12,padding:10,background:\"#fff3e0\",border:\"1px solid #FFB74D\",borderRadius:4}}>
            ...
            <div style={{fontSize:10,color:\"#666\",marginTop:2}}>Average: {result.anomCheck.avg?.toFixed(2) || \"N/A\"} {getUtil(utilId).unit}/day</div>
...
function Onboarding({ onDone }) {
  ...
  const isSetup = slide === ONBOARD_SLIDES.length;
  ...
  const allSlides = ONBOARD_SLIDES.length + 1;
...
function SubmitScreen({ u, selUtil, setSelUtil, aiOk, setAiOk, reading, setReading, prevRead, setPrevRead, busy, usage, reward, handleSubmit, verifyKey, wallet, setShowWallet, subs, T, setTab }) {
  ...
  return (
    <>
      ...
      <div className="form-card" style={{"--uc":T[u.id]||T.electric,"--ubg":T[u.id+"Bg"]||T.electricBg,"--uborder":T[u.id+"Border"]||T.electricBorder,marginTop:14}}>
        {!wallet
    ...
    <>
      ...
      {sorted.map((item, i) => (
        <div key={item.addr} className={`lb-item ${item.isMe?"me":""}`}>
          <div className={`lb-rank ${i<3?"top":""}`}>{item.rank}</div>
  ...
  return (
    <>
      ...
      <div className="pstat-row">
        {[{v:b3tr.toFixed(2),k:"B3TR Earned"},{v:subs.length,k:"Submissions"}].map(x=>(<div key={x.k} className="pstat"><div className="pstat-val">{x.v}</div><div className="pstat-key">{x.k}</div></div>))}
      </div>
      ...
      <div className="setting-row" onClick={() => generateMonthlyPDF(b3tr, subs)}>
        <div className="sr-left">
          ...
          <div><div className="sr-label">Download Monthly Report</div><div className="sr-sub">PDF with stats, trends, and proof of tracking</div></div>
        </div>
        <div className="sr-right"><span style={{fontSize:11,fontWeight:700,color:"#2E7D32"}}>Export</span></div>
      </div>
      ...
      <div className="notif-card">
        ...
        {[
          ...
          <div key={n.id} className="notif-row">
            ...
            <Toggle on={notifs[n.id]} onToggle={()=>setNotifs(prev=>({...prev,[n.id]:!prev[n.id]}))}/>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    const seenIntro = localStorage.getItem('greenlog_seen_intro');
    if (!seenIntro) {
      setShowIntro(true);
    }
          ...
          <div className="hdr">
            <div className="logo">
              <div className="logo-mark">
                <svg viewBox="0 0 28 28" fill="none" width="14" height="14"><path d="M14 4C9 4 5 8 5 13c0 3.6 2 6.7 5 8.3V24h8v-2.7c3-1.6 5-4.7 5-8.3 0-5-4-9-9-9z" fill="currentColor" opacity="0.9"/><path d="M11 13c0-1.7 1.3-3 3-3s3 1.3 3 3" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" opacity="0.7"/></svg>
              </div>
              <div>
            ...
            <div className="hdr-actions">
              <div className="dark-toggle" onClick={()=>setDark(d=>!d)}>
                {dark
                  ...
                  : <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" width="14" height="14"><path d="M17 11.5A7 7 0 119.5 3a5 5 0 007.5 8.5z" fill="currentColor" opacity="0.15"/><path d="M17 11.5A7 7 0 119.5 3a5 5 0 007.5 8.5z"/></svg>
                }
              </div>
